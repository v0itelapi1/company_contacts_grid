...
$APPLICATION->SetAdditionalCSS("/bitrix/themes/.default1/crm-entity-show.css");
if(SITE_TEMPLATE_ID === 'bitrix24')
{
	$APPLICATION->SetAdditionalCSS("/bitrix/themes/.default1/bitrix24/crm-entity-show.css");
}
...
//korepanov 12.02.2024
if(!empty($arResult['HEADERS']['UF_OFFICE']))
{
    $arIds = [];
    ...
    if(!empty($arIds))
	{
        $dataContact = (new \Bitrix\Main\Entity\Query(\Bitrix\Crm\ContactTable::getEntity()))
            ->setSelect(
                [
                    'ID',
                    'UF_OFFICE',
		...
		{	
			if($contact['UF_OFFICE'] != '')
			{
				$res = CIBlockElement::GetByID($contact['UF_OFFICE']);
				if($ar_res = $res->GetNext())
					
				$arFixOfficeField[$contact['ID']] = $ar_res['NAME'];
			}
			else
			{
				$arFixOfficeField[$contact['ID']] = "Без отдела";
			}
			
			
        }
		
		foreach($arResult['GRID_DATA'] as $k => $item)
        {
            $arResult['GRID_DATA'][$k]['data']['UF_OFFICE'] = $arFixOfficeField[$item['id']];
		}
...
foreach($arResult['GRID_DATA'] as $item)
{	
	$newGroupId = $item['data']['UF_OFFICE'];
    $officeName = $item['columns']['UF_OFFICE'] ?? '-';
	
    if($prevGroupId == '-' || $prevGroupId != $newGroupId)
	{
        $resultItem = [];
        $resultItem['group_id'] = $newGroupId;
        $resultItem['has_child'] = true;
        $resultItem['not_count'] = true;
        $resultItem['custom'] = "<div style='text-align: left;font-size: 14px;font-weight: bold;'>{$newGroupId}</div>";
		
        $printGr = $newGroupId ?? '-';
        $resultItem['attrs'] = [
            'data-type' => 'group',
            'data-group-id' => $printGr,
        ];

        $printGr = $newGroupId ?? '-';

        $resultItem['attrs_string'] = 'data-type="group"data-group-id="'.$printGr.'"';

        $arNewItems[] = $resultItem;
        unset($resultItem);
    }
...
$arResult['GRID_DATA'] = $arNewItems;
//end korepanov 12.02.2024
